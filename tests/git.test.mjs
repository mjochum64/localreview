...
test("dirty working tree resolves to a working-tree review", () => {
  const cwd = makeTempDir();
  initGitRepo(cwd);
  fs.writeFileSync(path.join(cwd, "a.txt"), "eins\n", "utf8");
  run("git", ["add", "."], { cwd });
  run("git", ["commit", "-m", "init"], { cwd });
  fs.writeFileSync(path.join(cwd, "a.txt"), "zwei\n", "utf8");
  ...
  const context = collectReviewContext(cwd, target, {});
  assert.ok(context.content.includes("## Git Status"));
